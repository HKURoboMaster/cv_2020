...
{
    uint16_t msg = 0x00ff & (uint16_t)(ch);
    uint16_t tmp = crc ^ msg;
    return (crc >> 8) ^ crc_tab16[tmp & 0xff];
}

uint32_t CRC32Update(uint32_t crc, uint8_t ch)
{
    uint32_t msg = 0x000000ffL & (uint32_t)(ch);
    uint32_t tmp = crc ^ msg;
    return (crc >> 8) ^ crc_tab32[tmp & 0xff];
}
